...
async def start_trade_monitor(trade_monitor):
    ...
    while True:
        try:
            if (
                hb.strategy_task is not None
                and not hb.strategy_task.done()
                and all(market.ready for market in hb.markets.values())
            ):
                with hb.trade_fill_db.get_new_session() as session:
                    ...
                    if trades:
                        ...
                        quote_assets = {t.symbol.split("-")[1] for t in trades}
